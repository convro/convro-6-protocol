...
-- ============================================================================
-- FEATURE: Conversations + Sealed Sender (v1.1)
-- ============================================================================
-- Ref: docs/server/CONVERSATIONS_SEALED_SENDER.md

-- ----------------------------------------------------------------------------
-- 1. Add privacy_mode to users table
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 2. Sealed Sender Messages Table
-- ----------------------------------------------------------------------------
-- ⚠️ PRIVACY: Sender identity hidden from server (only recipient visible)
-- Ref: docs/server/CONVERSATIONS_SEALED_SENDER.md, Part 2
...
-- Comments
COMMENT ON TABLE messages_sealed IS '⚠️ SEALED SENDER: Sender identity hidden from server - only recipient visible for routing';
COMMENT ON COLUMN messages_sealed.encrypted_envelope IS 'Contains encrypted sender info + message (64KB padded for privacy)';
COMMENT ON COLUMN messages_sealed.created_at IS 'Rounded to 5min intervals for timing obfuscation';

-- ----------------------------------------------------------------------------
-- 3. Conversations Materialized View
-- ----------------------------------------------------------------------------
-- Purpose: User-facing conversation list (like WhatsApp/Signal)
-- Ref: docs/server/CONVERSATIONS_SEALED_SENDER.md, Part 1

CREATE MATERIALIZED VIEW IF NOT EXISTS user_conversations AS
SELECT
    -- Conversation identity (unique per user pair)
    CASE
...
COMMENT ON MATERIALIZED VIEW user_conversations IS 'User conversation list (like WhatsApp/Signal) - aggregates sessions + messages';

-- ----------------------------------------------------------------------------
-- 4. Sealed Sender Functions
-- ----------------------------------------------------------------------------

-- Function: Cleanup expired sealed messages
...
COMMENT ON FUNCTION cleanup_expired_sealed_messages() IS 'Delete delivered sealed messages older than expires_at (privacy protection)';

-- Function: Refresh conversations view (triggered by message insert)
...
EXCEPTION
    WHEN OTHERS THEN
        -- If concurrent refresh fails (e.g., no unique index), log but don't fail transaction
        RAISE WARNING 'Failed to refresh user_conversations: %', SQLERRM;
        RETURN NEW;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION refresh_conversations_materialized_view() IS 'Refresh conversations view on message insert/update';

-- ----------------------------------------------------------------------------
-- 5. Triggers for Sealed Sender
-- ----------------------------------------------------------------------------

-- Immutability trigger for sealed messages
...
-- Refresh conversations on message changes (runs after statement, not per row)
-- Note: This may be expensive - consider using cron job instead for production
...
-- Grant permissions on new tables/views to convro_app role
DO $$ BEGIN
    -- Sealed messages table
    GRANT SELECT, INSERT, UPDATE ON messages_sealed TO convro_app;
    GRANT DELETE ON messages_sealed TO convro_app;
    ...
    RAISE NOTICE '✅ Sealed sender + conversations permissions granted to convro_app';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE '⚠️  Some permissions already exist or role not found: %', SQLERRM;
END $$;
